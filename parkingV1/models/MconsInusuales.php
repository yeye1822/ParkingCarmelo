...
class conInusuales{
	public function mostrarDatosInusuales(){
		...
		$date = date('Y-m-d');
		$sql = "SELECT fechaRegistro,placaInusual,idRegistro FROM registrocarro WHERE idCarroPK = 2 AND Inusual = 'Inusual' AND idConceptoFK = 1 and DATE(fechaRegistro) = '$date'";
